CREATE MATERIALIZED VIEW IF NOT EXISTS public.patient_view
AS
SELECT
  r.id AS patient_id,
...
FROM mhea_replica.registration AS r
INNER JOIN mhea_replica.registration_address AS ra
  ON r.id = ra.registration_id
JOIN mhea_replica.am_city as tc
 on ra.present_city_id = tc.id
JOIN mhea_replica.am_state as ts
	on ra.present_state_id = ts.id
JOIN mhea_replica.am_nationality as tn
	on ra.present_country_id = tn.id
WITH NO DATA;
